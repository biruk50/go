...
func NewController(u Usecases.UserUsecase, b Usecases.BudgetUsecase, c Usecases.CashRequestUsecase, e Usecases.ExpenseUsecase, r Usecases.ReportUsecase, t Usecases.TaskUsecase, jwtSvc Infrastructure.JWTService) *Controller {
	return &Controller{UserUC: u, BudgetUC: b, CashRequestUC: c, ExpenseUC: e, ReportUC: r, TaskUC: t, JWT: jwtSvc}
}
...
// Budget handlers
func (ctr *Controller) GetAllBudgets(c *gin.Context) {
	budgets, err := ctr.BudgetUC.GetAllBudgets()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (ctr *Controller) GetBudgetByID(c *gin.Context) {
	id := c.Param("id")
	b, err := ctr.BudgetUC.GetBudgetByID(id)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
		return
	}
...
func (ctr *Controller) GetBudgetSummary(c *gin.Context) {
	id := c.Param("id")
	summary, err := ctr.BudgetUC.GetBudgetSummary(id)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
		return
	}
...
func (ctr *Controller) CreateBudget(c *gin.Context) {
	...
	created, err := ctr.BudgetUC.CreateBudget(&payload)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (ctr *Controller) UpdateBudget(c *gin.Context) {
	id := c.Param("id")
	var payload Domain.Budget
	if err := c.ShouldBindJSON(&payload); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
	...
	if err := ctr.BudgetUC.UpdateBudget(id, &payload); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "updated"})
...
func (ctr *Controller) ApproveBudget(c *gin.Context) {
	id := c.Param("id")
	if err := ctr.BudgetUC.ApproveBudget(id); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (ctr *Controller) RejectBudget(c *gin.Context) {
	id := c.Param("id")
	if err := ctr.BudgetUC.RejectBudget(id); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
